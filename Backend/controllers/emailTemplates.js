...
const otpTemplate = (user, otp) => {
  return [
    `OTP for AEC Coding Club Registration`,
    `<body>
  <div>
...
const notifyall = (event, date, url) => {
  return [
    `Announcement from AECCC`,
    `Dear participant ${event} is going to start from ${date} link of the event ${url}`,
  ];
};

const resettoken = (url, uid) => {
  return [
    `Reset Password (AECCC)`,
    `Hello, You Requested for a Password Reset Please Click On the Link to Reset Password ${url}
  <br
  >UID : ${uid}`,
  ];
};
