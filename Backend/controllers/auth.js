...
exports.register = async (req, res) => {
  try {
    ...
    const { firstName, lastName, email, password, contact_no, batch, branch, confirmPassword, linkedin, github } =
      req.body
    ...
    try {
      ...
      console.log((Date.now() - user_notActive.timeStamp) / (1000 * 24 * 60 * 60))
      if (user_notActive) {
        console.log('After find')
        if ((Date.now() - user_notActive.timeStamp) / (1000 * 24 * 60 * 60) >= 1) {
    ...
    if (!count) {
      ...
      countupdate = await Counter.findByIdAndUpdate({ _id: countfreshfind._id }, { $inc: { seq: 1 } })
      console.log(countupdate)
    } else {
      console.log('Here is my Counter : ', count)
      countupdate = await Counter.findByIdAndUpdate({ _id: count._id }, { $inc: { seq: 1 } })
      console.log(countupdate)
    }
    ...
    const notActive = await Counter.updateOne({ branch: branch, batch: batch }, { $push: { notActive: user.uid } })
    console.log(notActive)
  ...
  if (otp) {
    ...
    if (docs.otpstatus && docs.active == false) {
      if (Date.now() - docs.timeStamp > 5 * 60 * 60 * 1000) {
        ...
        User.updateOne({ uid: uid }, { $set: { 'otpstatus.wrongTry': docs.otpstatus.wrongTry + 1 } })
          .then((msg) => {})
          .catch((err) => {
            console.log(err)
          })
        ...
        console.log(' +++++++++++++++++++++++====================++++++++++++++++++++++')
        Counter.updateOne({ branch: userupdate.branch, batch: userupdate.batch }, { $pull: { notActive: uid } })
          .then(() => {
            console.log(`notActive uid: ${uid} found in ${userupdate.branch}-${userupdate.batch}`)
          })
          .catch(() => {})
        // } catch (error) {
        ...
        return res.status(200).send({ success: true, token: true, message: 'account activated' })
      }
    } else {
